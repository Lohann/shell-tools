...
Non portable grammas such as substitution `$(command)` and arithmetic expansion `$(( 1 + 2 ))`, or `zsh` and `bash` specific expansions such as `${name::=word}`, `${name:offset:length}`, `${(S)foo//${~sub}/$rep}`, are not portable and must be avoided, if you wish to use them after detect their support in the current shell, they must be defined inside `eval` because shells that don't support them may parse the script wrongly or crash.
